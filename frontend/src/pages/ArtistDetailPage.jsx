...
const SnsLinkIcon = ({ url }) => {
  ...
  if (url.includes("instagram.com")) {
    ...
    colorClass = "text-gray-600 hover:text-gray-700";
  ...
  return (
    <a
      ...
      className={`inline-flex items-center gap-2 font-medium transition-colors ${colorClass}`}
    >
      {icon}
      <span className="hidden sm:inline">{name} 바로가기</span>
...
const ArtistDetailPage = () => {
  ...
  useEffect(() => {
    const fetchSchedules = async () => {
      ...
      try {
        ...
        setScheduleError(
          err.message || "스케줄 정보를 불러오는 데 실패했습니다."
        );
      } finally {
  ...
  useEffect(() => {
    const fetchTracks = async () => {
      if (!artistId) return;
      try {
        const trackList = await artistService.getArtistTracks(artistId);
        setTracks(trackList);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    ...
    const checkAdminStatus = async () => {
      try {
        const isAdmin = await artistService.checkIfAdmin(
          artist.managerId,
          currentUser.id
        );
        setIsAdmin(isAdmin);
  ...
  if (loading)
    return (
      <div className="text-center mt-20 text-xl text-indigo-600">
        로딩 중...
      </div>
    );
  if (error)
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 md:p-8 bg-gray-50 min-h-screen">
      {infoMessage.message && (
        <div
          className={`p-4 mb-6 rounded-lg shadow-md font-medium ${
        ...
        <div className="w-full md:w-1/3 flex-shrink-0 flex flex-col items-center p-4">
          ...
          {isLoggedIn ? (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-full font-bold mt-3 transition-colors duration-200
                            ${
                              isFollowing
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          ...
          <ul className="space-y-2">
            {tracks.map((track, index) => (
              ...
              >
                <span className="w-8 text-center text-indigo-600 font-bold">
                  {index + 1}
                </span>
