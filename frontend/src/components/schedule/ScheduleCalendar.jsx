...
function ScheduleCalendar({ schedules, isLoading, error }) {
  ...
  const selectedDayDetails = validSchedules
    .filter(
      (schedule) =>
        new Date(schedule.liveDate).toDateString() ===
  ...
  if (isLoading)
    return (
      <div className="text-center py-10 text-gray-500">스케줄 로드 중...</div>
    );
  if (error)
